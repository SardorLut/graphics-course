...
void App::drawFrame()
{
  ...
  if (nextSwapchainImage)
  {
    ...
    ETNA_CHECK_VK_RESULT(currentCmdBuf.begin(vk::CommandBufferBeginInfo{}));
    {
      ...
      // Map the constant buffer and copy the parameters
      std::byte* data = constantBuf[bufIndex].map();
      std::memcpy(data, &params, sizeof(params));
      constantBuf[bufIndex].unmap();

      // Flush barriers to ensure proper synchronization
      etna::flush_barriers(currentCmdBuf);

      // Get the shader program for the image
      auto imgInfo = etna::get_shader_program("image");

      // Create a descriptor set for the graphics pipeline
      ...
      // Record commands to render the frame
      {
        ETNA_PROFILE_GPU(currentCmdBuf, "Render target");

        // Set up the render target state
      ...
      // Flush barriers again to ensure proper synchronization
      etna::flush_barriers(currentCmdBuf);

      // Update the buffer index for the next frame
